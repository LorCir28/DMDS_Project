...
from states s join countries co on s.country_id = co.id join cities c 
	on c.state_id = s.id
where co.name = 'Italy' and 
	(s.type = 'region' or s.type = 'autonomous region') 
	and c.name in (select st.name
				from countries cou join states st on st.country_id = cou.id
				where cou.name = 'Italy' 
					and (st.type = 'province' or 
						 st.type = 'decentralized regional entity'))												
	and c.latitude > all (select cit.latitude
						from countries coun join cities cit on 
						  cit.country_id = coun.id
