...
from cities c join countries co on c.country_id = co.id join subcontinents sr
		on co.subcontinent_id = sr.id
...
select distinct c.name as city_name, co.currency_name as currency, sr.name
from cities c join states s on c.state_id = s.id join countries co on co.id = c.country_id
...
select distinct c.name as city_name, co.currency_name as currency, sr.name
from cities c join countries co on c.country_id = co.id join subcontinents sr 
	on co.subcontinent_id = sr.id join states s on c.state_id = s.id
where s.type not like 'autonomous%' and sr.name like 'South%'
